...
# # # Section headings
# # SECTION_HEADINGS = [
# #     "abstract", "I. INTRODUCTION", "methods", "materials and methods", 
...
# from pymongo import MongoClient
# import os
# import re   # 🔹 ADDED: for flexible heading detection
# from pydantic import BaseModel
...
# # 🔹 NEW: Helper function to flexibly match section headings
# def match_heading(line: str):
#     line_clean = line.strip().lower()
#     for h in SECTION_HEADINGS:
#         # Match patterns like "1. Introduction", "I. INTRODUCTION", etc.
...
#         with pdfplumber.open(pdf_file) as pdf:
#             for page_num, page in enumerate(pdf.pages, start=1):
#                 # 🔹 UPDATED: Add OCR fallback if text extraction fails
#                 text = page.extract_text()
#                 if not text or text.strip() == "":
#                     print(f"⚠️ No text layer on page {page_num}, using OCR...")
...
#         # 🔹 Debug log: print first part of extracted text
...
#         current_text = ""
#         for line in full_text.split("\n"):
#             if match_heading(line):   # 🔹 UPDATED: use flexible regex match
#                 if current_heading:
#                     sections.append({"heading": current_heading, "text": current_text.strip()})
#                 current_heading = line.strip()
#                 current_text = ""
...
#         # 4️⃣ Title & Abstract (Improved fallback)
#         if sections:
#             parsed_json["title"] = sections[0]["text"].split("\n")[0][:150]
...
#             parsed_json["title"] = full_text.split("\n")[0][:150]
#         if not parsed_json["abstract"]:
#             abstract_match = re.search(r"abstract[:\n\s]+(.*?)(\n[A-Z]|$)", full_text, re.IGNORECASE | re.DOTALL)
#             if abstract_match:
#                 parsed_json["abstract"] = abstract_match.group(1).strip()

#         # 5️⃣ Authors (rough extraction improvement)
#         if sections and parsed_json["title"]:
#             lines = full_text.split("\n")
#             title_line_index = next((i for i, l in enumerate(lines) if parsed_json["title"].strip() in l), None)
#             if title_line_index is not None:
#                 parsed_json["authors"] = [l.strip() for l in lines[title_line_index + 1 : title_line_index + 5] if l.strip()]
